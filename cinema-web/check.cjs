...
const file=__dirname+'/index.html';const html=fs.readFileSync(file,'utf8');const script=html.match(/<script>([\s\S]+)<\/script>/)[1];
const listeners={};const elements=new Map();function element(){return {innerHTML:'',textContent:'',style:{},classList:{add(){},remove(){},toggle(){}},append(){},remove(){},focus(){},click(){},replaceChildren(){}};}
const document={title:'',activeElement:null,body:{style:{}},getElementById(id){if(!elements.has(id))elements.set(id,element());return elements.get(id);},addEventListener(type,fn){(listeners[type]??=[]).push(fn);},querySelector(){return null;},querySelectorAll(){return [];},createElement(){return element();}};
