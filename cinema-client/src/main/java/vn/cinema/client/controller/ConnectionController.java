...
public final class ConnectionController {
 ...
 private CinemaApp app;private TcpClient client;
 ...
 @FXML private void connect(){
  ...
  CompletableFuture.supplyAsync(()->{try{return new TcpClient(host,port);}catch(Exception e){throw new CompletionException(e);}})
   .whenComplete((connected,error)->Ui.run(()->{
    ...
    client.onDisconnect(message->Ui.run(()->{loginBox.setDisable(true);statusLabel.setText(message);}));
 ...
 @FXML private void login(){
  ...
  client.request("LOGIN",Json.obj("username",usernameField.getText(),"password",passwordField.getText()))
   .whenComplete((value,error)->Ui.run(()->{busy(false);if(error!=null)statusLabel.setText(Ui.message(error));else{passwordField.clear();app.loggedIn(client,value.getAsJsonObject().getAsJsonObject("user"));}}));
 }
